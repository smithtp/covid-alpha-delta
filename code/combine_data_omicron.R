# combine Omicron STP Rts with climate variables

source("code/packages.R")

STP_cases <- readRDS("data/omicron_cases.rds")

STP_rts <- readRDS("data/transmission_individual_results_omicron.rds")

STP_rts <- left_join(STP_rts, STP_cases, by = c("area", "epiweek")) %>%
  mutate(omicron_proportion = corrected_negative/(corrected_negative+corrected_positive))
...
STP_rts <- STP_rts %>%
  select(-P_neg_greater, -Difference, -P_Difference, -P_Ratio, -`R(S-)LI`, -`R(S-)UI`,
         -`R(S+)LI`, -`R(S+)UI`) %>%
  rename(omicron_Rt = `R(S-)`, delta_Rt = `R(S+)`, omicron_cases = corrected_negative, delta_cases = corrected_positive)
...
ltla_vacc <- read.csv("data/ltla-vacc-2022-07-01.csv")
...
summ_vacc <- ltla_vacc %>%
  ...
  dplyr::summarize(first_dose = max(cumPeopleVaccinatedFirstDoseByVaccinationDate),
                   ...
                   third_dose = max(cumPeopleVaccinatedThirdInjectionByVaccinationDate))
...
# now get % of population vaccinated (lets go with 2nd dose received here - can discuss later)
STP_rts$full_vax_rate <- STP_rts$third_dose/STP_rts$Population

## ---- add in NPI data ------ #

# read npis
npi_data <- read.csv("data/oxford-interventions-20220706.csv")
england_npis <- npi_data[npi_data$RegionName == "England",]
england_npis$Date <- as.Date(as.character(england_npis$Date), format = "%Y%m%d")

# chop to 2021
england_npis <- england_npis %>%
...
# summarize by week
summ_npi <- england_npis %>% 
  select(Date, StringencyIndex, GovernmentResponseIndex, ContainmentHealthIndex,
...
# save the outputs
write.csv(STP_rts, "data/omicron_delta_climate.csv", row.names = FALSE)
